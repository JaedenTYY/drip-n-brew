...
/**
 * handleOrderComplete: 
 * Re-integrated success logic directly in the drawer.
 */
const handleOrderComplete = (orderId: string, customerName: string) => {
  console.log('[CartDrawer] SUCCESS SIGNAL RECEIVED!', { orderId, customerName })
  
  lastOrderId.value = orderId
  lastCustomerName.value = customerName
  
  // 1. Show popup instantly
  showSuccess.value = true
  
  // 2. Hide checkout inputs
  showCheckout.value = false
  
  // 3. Clear cart AFTER a delay so popup stays visible
  setTimeout(() => {
    console.log('[CartDrawer] Redirecting to tracker...')
...
<template>
  <div>
      ...
      >
        ...
        >
          ...
          <div class="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar">
            <!-- We only show "Empty" if cart is empty AND we aren't in success mode -->
                ...
                <div class="flex flex-1 flex-col justify-between py-0.5">
                  ...
                  <div class="flex items-center justify-between mt-4">
                    <div class="flex items-center gap-1 bg-gray-50 rounded-xl p-1 border border-gray-100">
                      <button @click="cartStore.updateQuantity(item.id, item.quantity - 1)" class="flex h-8 w-8 items-center justify-center rounded-lg bg-white shadow-sm text-gray-400 hover:text-gray-900 transition-all"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M20 12H4" /></svg></button>
                      <span class="w-8 text-center text-xs font-black tabular-nums">{{ item.quantity }}</span>
                      <button @click="cartStore.updateQuantity(item.id, item.quantity + 1)" class="flex h-8 w-8 items-center justify-center rounded-lg bg-white shadow-sm text-gray-400 hover:text-gray-900 transition-all"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 4v16m8-8H4" /></svg></button>
                    </div>
                    <button @click="cartStore.removeItem(item.id)" class="text-[10px] font-black text-gray-400 hover:text-red-500 uppercase tracking-widest px-2 py-1 transition-colors">Remove</button>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="cartStore.items.length > 0" class="border-t border-gray-100 p-6 pb-10 sm:pb-6 bg-white flex-shrink-0">
            <div v-if="!showCheckout" class="space-y-5">
              ...
              <button @click="toggleCheckout" class="w-full bg-orange-600 py-5 rounded-2xl text-center font-black uppercase tracking-widest text-white shadow-xl shadow-orange-200 hover:bg-orange-700 transition-all">Checkout Now</button>
            </div>
            ...
            <div v-else class="animate-in slide-in-from-bottom-4 duration-300">
              <div class="mb-6 flex items-center justify-between">
                <button @click="toggleCheckout" class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-gray-900 group">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 group-hover:-translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M15 19l-7-7 7-7" /></svg>
                  Back to Cart
                </button>
    ...
    <!-- Global Success Popup: Now at the bottom of the root div for high priority -->
    <SuccessPopup 
      :show="showSuccess" 
...
.custom-scrollbar::-webkit-scrollbar-thumb { @apply bg-gray-100; border-radius: 10px; }
</style>
